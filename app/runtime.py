...
def _pick_query_param_key(keys: list[str], endpoint_name: str, flow_text: str, selected: str) -> str | None:
    ...
    # Filter out system/config parameters; user selections should never go into these
...
def _build_query_for_selection(url: str, endpoint_name: str, flow_text: str, selected: str) -> dict[str, str] | None:
    ...
    # Never map user selections into ignored/system keys such as the DB name
...
def _auto_postman_get(module: Any, user_input: str) -> str:
    ...
    if not callable(postman_call):
        return ""
    endpoint_name = str(best.get("name") or "")
    endpoint_url = str(best.get("url") or "")
    ...
    if callable(parse_payload) and callable(format_list):
        try:
            ...
            if formatted:
                runtime_ctx = getattr(module, "RUNTIME_CONTEXT", None)
                if isinstance(runtime_ctx, dict):
                    runtime_ctx["last_options_endpoint"] = endpoint_name
                    runtime_ctx["last_options_url"] = endpoint_url
                return formatted
        except Exception:
            pass
...
def _auto_postman_from_flow_selection(module: Any, messages: list[dict[str, str]] | None, user_input: str) -> str:
    ...
    options, _, _, _ = _find_last_options_context(messages)
    if not options:
        return ""
    ...
    if options_are_services:
        return ""
    runtime_ctx = getattr(module, "RUNTIME_CONTEXT", None)
    last_options_endpoint = ""
    if isinstance(runtime_ctx, dict):
        last_options_endpoint = str(runtime_ctx.get("last_options_endpoint") or "").strip()

    flow_map = _flow_step_endpoints(requests, flow_text) if flow_text else []
    ordered_endpoints: list[dict[str, Any]] = []
    last_name = ""
    for _, mapped_endpoint in flow_map:
        if not mapped_endpoint:
            continue
        name = str(mapped_endpoint.get("name") or "").strip()
        if not name:
            continue
        if name.lower() == last_name:
            continue
        ordered_endpoints.append(mapped_endpoint)
    ...
    if not ordered_endpoints:
        for req in requests:
            if not isinstance(req, dict):
                continue
            name = str(req.get("name") or "")
            if re.search(r"(login|auth|token)", name, flags=re.I):
                continue
            ordered_endpoints.append(req)

    next_endpoint = None
    if last_options_endpoint and ordered_endpoints:
        for idx, mapped_endpoint in enumerate(ordered_endpoints):
            if str(mapped_endpoint.get("name") or "").lower() == last_options_endpoint.lower():
                for candidate in ordered_endpoints[idx + 1:]:
                    if str(candidate.get("name") or "").lower() != last_options_endpoint.lower():
                        next_endpoint = candidate
                        break
                break
    if next_endpoint is None and ordered_endpoints:
        selection_count = len(_selection_history(messages)) + 1
        get_indices = [
            idx
            for idx, mapped_endpoint in enumerate(ordered_endpoints)
            if str(mapped_endpoint.get("method", "GET")).upper() == "GET"
        ]
        if get_indices and selection_count <= len(get_indices):
            idx = get_indices[selection_count - 1]
            for candidate in ordered_endpoints[idx + 1:]:
                if str(candidate.get("name") or "").lower() != str(ordered_endpoints[idx].get("name") or "").lower():
                    next_endpoint = candidate
                    break
    ...
    if endpoint is None:
        selection_input = (
            ...
            f"Last options endpoint: {last_options_endpoint}"
        )
        endpoint = _llm_select_endpoint(
            requests,
            selection_input,
    ...
    if callable(parse_payload) and callable(format_list):
        try:
            ...
            if formatted:
                if isinstance(runtime_ctx, dict):
                    runtime_ctx["last_options_endpoint"] = endpoint_name
                    runtime_ctx["last_options_url"] = url
                return formatted
        except Exception:
            pass
    return str(raw or "").strip()
...
def run_agent(
    ...
    if hasattr(module, "run_with_history"):
        output = module.run_with_history(messages or [], user_input)
        return output
    flow_text = ""
    services_raw = None
...
def run_agent_stream(
    ...
    messages = _sanitize_messages_runtime(messages)
    messages = _maybe_add_rag_context(module, device_id, name, user_input, messages)
    if hasattr(module, "run_with_history_stream"):
        return module.run_with_history_stream(messages or [], user_input), module
    flow_text = ""
    services_raw = None
    if isinstance(getattr(module, "AGENT_CONTEXT", None), dict):
        flow_text = str(module.AGENT_CONTEXT.get("flow") or "")
