...
async def run_agent_realtime_websocket(websocket: WebSocket, name: str) -> None:
    ...
    def query_value(*keys: str) -> str | None:
        ...
        async def stream_audio_from_text(response_text: str, payload: dict[str, Any]) -> bool:
            ...
            tts_instructions = (
                "You are a text-to-speech engine. Speak the input text verbatim without adding, removing, or "
                "paraphrasing any words. Do not answer or interpret. Only read the text exactly."
            )
    ...
    while True:
        ...
        if tts_only and speak_text:
            audio_sent = await stream_audio_from_text(speak_text, payload)
            await websocket.send_text(
                json.dumps(
                    {
                        "type": "done",
                        "output": speak_text,
                        "audio_transcript": speak_text,
        ...
        if is_audio:
            ...
            try:
                async with websockets.connect(
                    ...
                    await rt.send(json.dumps(session_update))
                    audio_b64 = str(audio_base64)
                    if _pcm_byte_len_from_base64(audio_b64) <= 0:
                        await websocket.send_text(
                            json.dumps(
                                {
                                    "type": "error",
                        ...
                        continue
                    await rt.send(json.dumps({"type": "input_audio_buffer.append", "audio": audio_b64}))
                    await rt.send(json.dumps({"type": "input_audio_buffer.commit"}))
                    await rt.send(json.dumps({"type": "response.create", "response": {"modalities": ["text", "audio"]}}))
                    ...
                    async for message in rt:
                        ...
                        if event_type in {"response.audio_transcript.done", "response.output_audio_transcript.done"}:
                            continue
                        if event_type.startswith("response") and "audio" not in event_type:
                ...
                usage = input_usage + output_usage
                if not audio_sent:
                    audio_sent = await stream_audio_from_text(final_output, payload)
        ...
        if _is_services_request(user_input):
            ...
            await stream_audio_from_text(response_text, payload)
            await websocket.send_text(json.dumps({"type": "done", "output": response_text}, ensure_ascii=False))
            continue
            ...
            if is_selection and last_options and not options_are_services:
                try:
                    ...
                    if flow_result:
                        ...
                        await stream_audio_from_text(flow_result, payload)
                        await websocket.send_text(json.dumps({"type": "done", "output": flow_result}, ensure_ascii=False))
                        continue
                except Exception:
                    pass
        ...
        if attachment_note:
            messages = [{"role": "system", "content": attachment_note}] + (messages or [])
        input_text = _build_realtime_input_text(user_input, messages) if user_input else ""
        audio_enabled = str(payload.get("audio_enabled") or "").strip().lower() in {"1", "true", "yes"}
        ...
        _write_agent_log(
            ...
            {
                "event": "user_prompt",
                "input": user_input or "voice_input",
                "messages": messages or [],
                "attachments": attachments,
            },
        )
        ...
        try:
            async with websockets.connect(
                ...
                if is_audio:
                    ...
                    await rt.send(json.dumps(append_event))
                    await rt.send(json.dumps({"type": "input_audio_buffer.commit"}))
                    response_create = {
                        "type": "response.create",
